...
export const catalogoService = {
  async getProductos() {
    ...
    return (data || []).map((p) => {
      ...
      else if (n.includes("gato") || n.includes("felin"))
        categoria = "Dieta Gatos";
  ...
  async getProductoCompleto(id) {
    ...
    const relaciones = productoIngredientes.filter(
      (pi) => pi.id_producto.toString() === id.toString(),
    );

    const composicion = relaciones.map((rel) => {
      const ingrediente = ingredientes.find(
        (i) => i.id.toString() === rel.id_ingrediente.toString(),
      );
      return {
        ...rel,
