  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold border ${borderColor} ${iconColor} bg-white`}
          >
...
function StockRow({ alerta }) {
  const pct = Math.min(
    100,
    Math.round((alerta.stockActual / Math.max(alerta.stockMinimo, 1)) * 100),
  );
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3.5">
      ...
      <div className="text-right shrink-0 ml-4">
        <p
          className={`text-lg font-extrabold ${agotado ? "text-red-600" : "text-orange-600"}`}
        >
          {alerta.stockActual}{" "}
          <span className="text-sm font-semibold">{alerta.unidad}</span>
        </p>
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3.5">
      <div>
        <p className="text-sm font-semibold text-black">{alerta.nombre}</p>
        <p className="text-xs text-gray-400 mt-0.5">
          Lote #{alerta.produccionId} · {alerta.cantidadProducida} uds
          producidas
        </p>
      </div>
      <div className="text-right shrink-0 ml-4">
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3.5">
      ...
      <div className="flex items-center gap-3 shrink-0 ml-4">
        ...
        <Link
          href={`/gestion/creditos/${alerta.creditoId}`}
...
export default function AlertasPage() {
  ...
  const stockIngredientes = alertas.filter(
    (a) => a.tipo === "stock_ingrediente",
  );
    ...
    <div className="space-y-6 text-black">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Monitorea problemas críticos que requieren atención.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {!loading && totalAlertas > 0 && (
            <div className="flex items-center gap-1.5 rounded-xl bg-red-50 px-3 py-1.5 border border-red-100 text-xs font-bold text-red-700">
              ...
              {totalAlertas} alerta{totalAlertas > 1 ? "s" : ""} activa
              {totalAlertas > 1 ? "s" : ""}
            </div>
          )}
          <button
      ...
      <div className="flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/50 px-4 py-3">
        ...
        <p className="text-xs text-blue-700 font-medium leading-relaxed">
          Vencimientos = lotes de producción dentro de {DIAS_ALERTA_VENCIMIENTO}{" "}
          días. Deudas = cuotas de clientes vencidas o que vencen en{" "}
          {DIAS_ALERTA_DEUDA} días. La campana notifica automáticamente 7 días
          antes.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          >
            {stockProductos.map((a) => (
              <StockRow key={a.id} alerta={a} />
            ))}
          </AlertCard>
          ...
          >
            {stockIngredientes.map((a) => (
              <StockRow key={a.id} alerta={a} />
            ))}
          </AlertCard>

          <AlertCard
            title={`Lotes por Vencer (${DIAS_ALERTA_VENCIMIENTO} días)`}
          ...
          >
            {vencLotes.map((a) => (
              <VencimientoRow key={a.id} alerta={a} />
            ))}
          </AlertCard>

          <AlertCard
            title="Deudas Vencidas"
            subtitle="Cuotas de clientes que ya pasaron su fecha de pago"
            icon={DollarSign}
            iconColor="text-red-600"
            bgColor="bg-red-50/40"
            borderColor="border-red-200"
          ...
          >
            {deudasVencidas.map((a) => (
              <DeudaRow key={a.id} alerta={a} />
            ))}
          </AlertCard>

          <AlertCard
            title={`Deudas por Vencer (${DIAS_ALERTA_DEUDA} días)`}
