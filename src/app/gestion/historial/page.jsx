...
export default function HistorialPage() {
  ...
  const { createMovimiento, creating } = useMovimientosProducto();
  ...
  const fetchHistorialData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      const [movIngredientes, movProductos, catIngredientes, catProductos] =
        await Promise.all([
      ...
      const listIngredientes = Array.isArray(movIngredientes)
        ? movIngredientes
        : [];
      ...
      const consolidated = [
        ...listIngredientes.map((m) => ({
          ...
          nombre:
            ingredientesMap[m.id_ingrediente]?.nombre ||
            `Ingrediente #${m.id_ingrediente}`,
        ...
        ...listProductos.map((m) => ({
          ...
          nombre:
            productosMap[m.id_producto]?.nombre || `Producto #${m.id_producto}`,
          unidad: "unidades",
          cantidad: parseFloat(m.cantidad || 0),
          stock_anterior: parseFloat(m.stock_anterior || 0),
          stock_posterior: parseFloat(m.stock_posterior || 0),
  ...
  const filteredMovimientos = movimientos.filter((item) => {
    ...
    if (filters.nombre) {
      ...
      if (
        ...
        !item.comentarios?.toLowerCase().includes(search)
      ) {
        return false;
      }
    }
    ...
    if (
      filters.movimiento !== "Todos" &&
      item.tipo_movimiento !== filters.movimiento
    ) {
      return false;
    }
  ...
  return (
    <div className="space-y-8 animate-fade-in text-black">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">
            Registro unificado de todos los movimientos de stock para
      ...
      <HistorialStats data={movimientos} loading={loading} />
      ...
      <HistorialTable data={filteredMovimientos} loading={loading} />
