...
const TIPO_CONFIG = {
  stock_producto: {
    icon: TrendingDown,
    color: "red",
    label: "Stock bajo · Producto",
  },
  stock_ingrediente: {
    icon: TrendingDown,
    color: "red",
    label: "Stock bajo · Ingrediente",
  },
  vencimiento_producto: {
    icon: Clock,
    color: "orange",
    label: "Vencimiento · Producto",
  },
...
function getTipoConfig(tipo) {
  return (
    TIPO_CONFIG[tipo] ?? { icon: AlertTriangle, color: "orange", label: tipo }
  );
}
        ...
        (() => {
          ...
          return (
            <div className="fixed top-5 right-5 z-[100] flex items-start gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3.5 shadow-xl max-w-xs w-full">
              <div
                className={`flex size-8 shrink-0 items-center justify-center rounded-xl border ${iconBg(cfg.color)}`}
              >
              ...
              {notifs.length === 0 ? (
                ...
                notifs.map((n) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex size-7 shrink-0 items-center justify-center rounded-lg border mt-0.5 ${iconBg(cfg.color)}`}
                      >
