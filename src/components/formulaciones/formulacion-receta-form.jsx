...
export default function FormulacionRecetaForm({
  ...
  const totalPorcentaje = useMemo(
    () =>
      ingredientesAgregados.reduce(
        (sum, ing) => sum + parseFloat(ing.porcentaje_ingrediente || 0),
        0,
      ),
  ...
  const canProceed = () => {
    ...
    if (step === 2)
      return ingredientesAgregados.length > 0 && totalPorcentaje <= 100;
    return true;
  };

  const handleAddIngrediente = () => {
    if (
      !tempIngrediente.id_ingrediente ||
      !tempIngrediente.cantidad_ingrediente ||
      !tempIngrediente.porcentaje_ingrediente
    ) {
    ...
    if (
      ingredientesAgregados.find(
        (i) => i.id_ingrediente === tempIngrediente.id_ingrediente,
      )
    )
      return setError("Ingrediente ya agregado");
    if (totalPorcentaje + pct > 100)
      return setError(`Excede 100%. Actual: ${totalPorcentaje.toFixed(2)}%`);
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex items-center flex-1">
                  <div
                    className={`size-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition ${s <= step ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-500"}`}
                  >
                    {s < step ? "✓" : s}
                  </div>
                  {s < 3 && (
                    <div
                      className={`flex-1 h-0.5 mx-1.5 transition ${s < step ? "bg-orange-500" : "bg-gray-200"}`}
                    />
                  )}
                </div>
              ))}
              ...
              <div className="space-y-3">
                    ...
                    <div className="col-span-6 sm:col-span-3">
                      <input
                        ...
                        onChange={(e) =>
                          setTempIngrediente({
                            ...tempIngrediente,
                            cantidad_ingrediente: e.target.value,
                          })
                        }
                        className="w-full rounded-lg border border-gray-300 px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                        placeholder="Cant."
                    ...
                    <div className="col-span-6 sm:col-span-3">
                      <input
                        ...
                        onChange={(e) =>
                          setTempIngrediente({
                            ...tempIngrediente,
                            porcentaje_ingrediente: e.target.value,
                          })
                        }
                        className="w-full rounded-lg border border-gray-300 px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                        placeholder="%"
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span
                      className={`text-xs font-bold ${totalPorcentaje > 100 ? "text-red-600" : "text-green-600"}`}
                    >
                  ...
                  {ingredientesAgregados.length === 0 ? (
                    ...
                    <div className="space-y-1.5 max-h-40 overflow-y-auto">
                      {ingredientesAgregados.map((ing, idx) => {
                        const data = ingredientes.find(
                          (i) => i.id === ing.id_ingrediente,
                        );
                        const esOriginal = originalIngredients.find(
                          (o) => o.id_ingrediente === ing.id_ingrediente,
                        );
                        return (
                          <div
                            key={idx}
                            className={`flex items-center justify-between p-2 rounded-lg border text-xs ${esOriginal ? "bg-white border-gray-200" : "bg-blue-50 border-blue-200"}`}
                          >
                  ...
                  {ingredientesAgregados.length > 0 && (
                    <div className="mt-2">
                      <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all ${totalPorcentaje > 100 ? "bg-red-500" : totalPorcentaje === 100 ? "bg-green-500" : "bg-orange-500"}`}
                          style={{
            ...
            {step === 3 && (
              <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-900">
                        {
                          productos.find((p) => p.id === productoSeleccionado)
                            ?.nombre
                        }
                      </p>
                    </div>
                ...
                <div className="space-y-1.5">
                  <h3 className="text-xs font-semibold text-gray-700">
                    Resumen ({ingredientesAgregados.length} ingredientes)
                  </h3>
                  {ingredientesAgregados.map((ing, idx) => {
                    const data = ingredientes.find(
                      (i) => i.id === ing.id_ingrediente,
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-500">
                            {ing.cantidad_ingrediente} {data?.unidad_medida}
                          </span>
                        </div>
                ...
                {mode === "edit" &&
                  originalIngredients.length > ingredientesAgregados.length && (
                ...
                {totalPorcentaje !== 100 && (
                  <div
                    className={`p-2 rounded-lg border text-xs ${totalPorcentaje > 100 ? "bg-red-50 border-red-200 text-red-700" : "bg-amber-50 border-amber-200 text-amber-700"}`}
                  >
                    {totalPorcentaje > 100
