...
export default function FormulacionRecetaForm({
  ...
  const totalPorcentaje = useMemo(() => {
    return ingredientesAgregados.reduce((sum, ing) => sum + parseFloat(ing.porcentaje_ingrediente || 0), 0);
  ...
  const handleAddIngrediente = () => {
    ...
    if (ingredientesAgregados.find(ing => ing.id_ingrediente === tempIngrediente.id_ingrediente)) {
      setError("Este ingrediente ya fue agregado");
      return;
    }

    if (totalPorcentaje + pct > 100) {
      setError(`Excederías el 100%. Total actual: ${totalPorcentaje.toFixed(2)}%`);
      return;
    }
  ...
  const handleSave = async () => {
    if (!productoSeleccionado || ingredientesAgregados.length === 0) {
      setError("Selecciona un producto y al menos un ingrediente");
      return;
    }
    ...
    try {
      ...
      if (results.some(r => !r.success)) {
        setError("Algunas relaciones no se pudieron crear");
      } else {
        onClose();
      }
    } catch (err) {
      ...
      <div className="flex min-h-full items-center justify-center p-3">
        ...
        <div className="relative w-full max-w-xl bg-white rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50">
            <div>
              <h2 className="text-base font-bold text-gray-900">Nueva Formulación</h2>
            ...
            <button
              onClick={onClose}
              disabled={loading}
              className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-200 transition cursor-pointer disabled:opacity-50"
            >
              <X className="size-4" />
            </button>
          </div>
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex items-center flex-1">
                  <div
                    className={`size-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition ${
                  ...
                  {s < 3 && (
                    <div
                      className={`flex-1 h-0.5 mx-1.5 transition ${
                ...
                <div className="space-y-1.5 max-h-80 overflow-y-auto pr-1">
                  {productos.map((p) => (
                    <button
                      ...
                      className={`w-full p-3 rounded-lg border-2 text-left transition cursor-pointer text-sm ${
                      ...
                      <div className="flex items-center justify-between">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500 mt-0.5">
                            Stock: {p.stock_actual} • ${parseFloat(p.precio).toLocaleString()}
                          </p>
                        </div>
                        {productoSeleccionado === p.id && (
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 rounded-lg bg-gray-50 border border-gray-200">
                  ...
                  <div className="grid grid-cols-12 gap-2">
                    <div className="col-span-12 sm:col-span-6">
                      <select
                        value={tempIngrediente.id_ingrediente}
                        onChange={(e) => setTempIngrediente({ ...tempIngrediente, id_ingrediente: e.target.value })}
                      ...
                      >
                        ...
                        {ingredientes
                          .filter(ing => !ingredientesAgregados.find(a => a.id_ingrediente === ing.id))
                          .map((ing) => (
                    ...
                    <div className="col-span-6 sm:col-span-3">
                      <input
                        ...
                        step="0.01"
                        min="0.01"
                        value={tempIngrediente.cantidad_ingrediente}
                        onChange={(e) => setTempIngrediente({ ...tempIngrediente, cantidad_ingrediente: e.target.value })}
                        className="w-full rounded-lg border border-gray-300 px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                        placeholder="Cant."
                      />
                    </div>
                    <div className="col-span-6 sm:col-span-3">
                      <input
                        ...
                        min="0.01"
                        max="100"
                        value={tempIngrediente.porcentaje_ingrediente}
                        onChange={(e) => setTempIngrediente({ ...tempIngrediente, porcentaje_ingrediente: e.target.value })}
                        className="w-full rounded-lg border border-gray-300 px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                        placeholder="%"
                      />
                    </div>
                  ...
                  <button
                    ...
                    disabled={!tempIngrediente.id_ingrediente || !tempIngrediente.cantidad_ingrediente || !tempIngrediente.porcentaje_ingrediente}
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className={`text-xs font-bold ${totalPorcentaje > 100 ? "text-red-600" : "text-green-600"}`}>
                  ...
                  {ingredientesAgregados.length === 0 ? (
                    ...
                    <div className="space-y-1.5 max-h-40 overflow-y-auto">
                      {ingredientesAgregados.map((ing, idx) => {
                        const ingredienteData = ingredientes.find(i => i.id === ing.id_ingrediente);
                        return (
                          <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-white border border-gray-200 text-xs">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <div className="min-w-0">
                                <p className="font-semibold text-gray-900 truncate">{ingredienteData?.nombre}</p>
                                <p className="text-gray-500">
                                  {ing.cantidad_ingrediente} {ingredienteData?.unidad_medida} • {ing.porcentaje_ingrediente}%
                                </p>
                              </div>
                            </div>
                            <button
                      ...
                      <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all ${
                            totalPorcentaje > 100 ? "bg-red-500" : totalPorcentaje === 100 ? "bg-green-500" : "bg-orange-500"
                          }`}
              ...
              <div className="space-y-3">
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-gray-50 text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-gray-900">{ingredienteData?.nombre}</span>
                          <span className="text-gray-500">{ing.cantidad_ingrediente} {ingredienteData?.unidad_medida}</span>
                        </div>
                ...
                {totalPorcentaje !== 100 && (
                  <div className={`p-2 rounded-lg border text-xs ${
                    ...
                    {totalPorcentaje > 100 
                      ? `⚠ Excede 100% (${totalPorcentaje.toFixed(1)}%)`
                      : `⚠ Incompleto: ${totalPorcentaje.toFixed(1)}%`
                    }
                  </div>
                )}
              </div>
