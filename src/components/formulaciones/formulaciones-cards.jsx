...
function getIngredientStyle(nombre) {
  ...
  if (name.includes("cereal"))
    return { icon: Wheat, hex: "#90BE6D", label: "Cereales" };

  if (name.includes("fruta") || name.includes("vegetal"))
    return { icon: Apple, hex: "#43AA8B", label: "Frutas/Veg" };

  if (name.includes("hueso carnoso de pollo"))
    return { icon: Bone, hex: "#8ECAE6", label: "Hueso Pollo" };

  if (name.includes("órganos") || name.includes("vísceras")) {
    if (name.includes("cerdo"))
      return { icon: Heart, hex: "#9D4EDD", label: "Vísceras Cerdo" };
    if (name.includes("cordero"))
      return { icon: Heart, hex: "#7B2CBF", label: "Vísceras Cordero" };
    if (name.includes("pollo"))
      return { icon: Heart, hex: "#C77DFF", label: "Vísceras Pollo" };
    if (name.includes("res"))
      return { icon: Heart, hex: "#5A189A", label: "Vísceras Res" };
  }

  if (name.includes("suplemento"))
    return { icon: Pill, hex: "#2A9D8F", label: "Suplementos" };
  if (name.includes("taurina"))
    return { icon: Zap, hex: "#277DA1", label: "Taurina" };
...
export default function FormulacionesCards({
  ...
  return (
    <div className="space-y-6">
      {Object.entries(formulacionesPorProducto).map(
        ([productoId, ingredients]) => {
          const producto = productosMap[productoId];
          const totalPorcentaje = ingredients.reduce(
            (sum, ing) => sum + parseFloat(ing.porcentaje_ingrediente),
            0,
          );
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        {ingredients.length} ingredientes • Total:{" "}
              ...
              <div className="px-6 py-4 bg-gray-50">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold uppercase tracking-wider text-gray-500">
                    Balance de la receta
                  </span>
                  <span
                    className={`text-sm font-bold ${totalPorcentaje === 100 ? "text-green-600" : "text-amber-600"}`}
                  >
                    {totalPorcentaje.toFixed(1)}%
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-3 mt-3">
                  {ingredients.map((ing) => {
                    const ingredienteData = ingredientesMap[ing.id_ingrediente];
                    const { hex, label } = getIngredientStyle(
                ...
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {ingredients.map((ing) => {
                    ...
                    const {
                      ...
                      hex,
                      label,
                    } = getIngredientStyle(ingredienteData?.nombre);
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-gray-900 text-sm truncate">
                              {ingredienteData?.nombre ||
                                `Ingrediente #${ing.id_ingrediente}`}
                            </p>
                            ...
                            <div className="mt-2 flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-sm font-bold text-gray-900">
                                  {parseFloat(ing.cantidad_ingrediente).toFixed(
                                    2,
                                  )}
                                </p>
                              </div>
