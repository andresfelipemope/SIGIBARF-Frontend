    ...
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50/50 p-8 text-center max-w-2xl mx-auto space-y-4">
        ...
        <p className="text-sm text-red-700 font-medium">
          {error || "No pudimos comunicarnos con el servidor. Revisa tu conexión de red."}
        </p>
        {onRetry && (
          <button
            onClick={onRetry}
  ...
  if (!movimientos || movimientos.length === 0) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center max-w-md mx-auto space-y-4">
        ...
        <h3 className="text-base font-bold text-black">No existen movimientos registrados</h3>
        <p className="text-xs text-gray-500 font-medium">
          Los movimientos creados manualmente o automáticos se verán reflejados en este panel histórico.
        </p>
      </div>
    );
  }
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-gray-100 text-xs font-bold uppercase tracking-wider text-gray-400 bg-gray-50/30">
              ...
              <th className="py-4 px-4 font-extrabold text-right w-36">Stock Ant/Post</th>
              <th className="py-4 px-4 font-extrabold w-40">Fecha</th>
              <th className="py-4 px-4 font-extrabold text-center w-16">Detalles</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {movimientos.map((mov) => {
              const prodName = mov.producto_nombre || mov.producto?.nombre || "Producto Desconocido";
              const prodId = mov.id_producto || mov.producto?.id || "-";
              ...
              return (
                ...
                >
                  ...
                  <td className="py-4 px-4 text-right text-xs font-semibold text-gray-500 font-mono">
                    {mov.stock_anterior !== null && mov.stock_anterior !== undefined ? mov.stock_anterior : "-"}
                    <span className="text-gray-300 mx-1">→</span>
                    <span className={
                      mov.tipo_movimiento?.toUpperCase() === 'ENTRADA' ? 'text-emerald-600 font-bold' :
                      mov.tipo_movimiento?.toUpperCase() === 'SALIDA' ? 'text-rose-600 font-bold' : 'text-amber-600 font-bold'
                    }>
                      {mov.stock_posterior !== null && mov.stock_posterior !== undefined ? mov.stock_posterior : "-"}
                    </span>
                  </td>
                  <td className="py-4 px-4 text-xs text-gray-600 font-medium">
                    {formatFecha(mov.fecha_registro || mov.created_at)}
                  </td>
